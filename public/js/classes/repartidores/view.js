...
import Repartidores from "./Repartidores.js";
...
let urls = new Urls();
let ui = new UI();
let urlsRepartidores = urls.repartidores;
let repartidores = new Repartidores(urlsRepartidores);

//========================= Html
let tableHTML = urls.repartidores.table;
let formHtml = urls.repartidores.form;

export function loadRepartidores() {
    let html = "";

    //========================= Cargamos la tabla y los datos de la tabla
    Promise.all([ui.loadTable(tableHTML), repartidores.getData()])
        .then((response) => {
    ...
    document.querySelectorAll(".btn-danger").forEach(function (item) {
        item.addEventListener("click", function () {
            ...
            if (confirm("Desea eliminar este registro")) {
                let promise = repartidores.delete(id);
                promise
                    .then(() => {
                        loadRepartidores();
                    })
...
function edit(id) {
    ...
    //========================= Form
    let form = document.querySelector("#form-repartidores");
    ...
    form.addEventListener("submit", function (e) {
        ...
        if (confirm(texto)) {
            let promise = repartidores.setOne(form);
            promise
                .then(() => {
                    loadRepartidores();
                })
